from sqlalchemy import (
    ...
    ForeignKey, Text, Enum, BigInteger, JSON
)
...
class DriverCashLog(Base):
    ...
    confirmed_by2 = Column(String, ForeignKey("users.id"), nullable=True)
    shift_id      = Column(String, ForeignKey("shifts.id"), nullable=True)
    created_at    = Column(DateTime(timezone=True), server_default=func.now())
...
class Package(Base):
    ...
    # Walk-in sender (when seller_id is null)
    ...
    # Status
    status            = Column(Enum(PackageStatusEnum), default=PackageStatusEnum.created)
    physical_location = Column(Enum(PhysicalLocationEnum), default=PhysicalLocationEnum.receiving)
...
class Dispute(Base):
    ...
    description     = Column(Text, nullable=False)
    status          = Column(Enum(DisputeStatusEnum), default=DisputeStatusEnum.open)
...
class PrintJob(Base):
    ...
    status       = Column(Enum(PrintJobStatusEnum), default=PrintJobStatusEnum.queued)
